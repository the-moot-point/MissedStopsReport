...
# Load Data
stops_report = pd.read_csv("Stops_Report.csv")
phases = pd.read_excel("/mnt/data/phases.xlsx")
region_lookup = pd.read_excel("/mnt/data/region lookup.xlsx")
invoices_report = pd.read_csv("/mnt/data/Invoices_Report.csv")
survey_report = pd.read_csv("/mnt/data/No_Sale_Survey.csv")

# Date for Yesterday
yesterday = datetime.now() - timedelta(days=1)
expected_day_of_sale = yesterday.strftime('%-m/%d/%Y') # Format date as m/dd/yyyy

# Phase and Day for Yesterday
phase_info_yesterday = phases[phases['Date'] == pd.Timestamp('2023-07-25')]
...
merged_report['Expected Day of Sale'] = expected_day_of_sale
...
net_sales = invoices_report[invoices_report['Date'] == expected_day_of_sale].groupby('Customer ID')['Total Cases'].sum().reset_index()
...
merged_report.to_csv('/mnt/data/Final_Report_Part2.csv', index=False)
